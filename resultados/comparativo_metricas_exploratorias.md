...
Valores extraídos da fase exploratória. As métricas com `type` comparam os clusters com a classe de ataque; as métricas com `label` comparam os clusters com a classe binária.
...
O notebook `codigo/COLAB_01_FASE_EXPLORATORIA.ipynb` reproduz a geração das tabelas e dos gráficos comparativos, salvando os artefatos em `resultados/exploratorio/` e `gráficos/exploratório/`.

Além das curvas por `k` do KMeans, o notebook gera:
